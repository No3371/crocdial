...
echo  =====================
echo.
croc send --code "$CodePhrase"$BatRelayFlag -- %*
goto :END
...
if [ `$# -eq 0 ]; then
    ...
    echo "  ====================="
    echo ""
    croc send --code "$CodePhrase"$ShRelayFlag -- "`$@"
fi
"@
...
>"%TMPFILE%" echo(!MSG!
croc send --code "$CodePhrase"$BatRelayFlag "%TMPFILE%"
goto :END
...
if [ `$# -eq 0 ]; then
    ...
    echo "`$MSG" > "`$TMPFILE"
    croc send --code "$CodePhrase"$ShRelayFlag "`$TMPFILE" </dev/null
    rm -f "`$TMPFILE"
else
    echo ""
    echo "  ========================"
