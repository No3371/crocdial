...
echo  =====================
echo.
croc$BAT_RELAY_FLAG --code "$CODE_PHRASE" send -- %*
goto :END
...
if [ \$# -eq 0 ]; then
    ...
    echo "  ====================="
    echo ""
    croc$SH_RELAY_FLAG --code "$CODE_PHRASE" send -- "\$@"
fi
SHEOF_END
...
>"%TMPFILE%" echo(!MSG!
croc$BAT_RELAY_FLAG --code "$CODE_PHRASE" send "%TMPFILE%"
goto :END
...
if [ \$# -eq 0 ]; then
    ...
    echo "\$MSG" > "\$TMPFILE"
    croc$SH_RELAY_FLAG --code "$CODE_PHRASE" send "\$TMPFILE" </dev/null
    rm -f "\$TMPFILE"
else
    echo ""
    echo "  ========================"
